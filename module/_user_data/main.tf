...
locals {
  int_linux_default_user_data = var.platform == "linux" && (var.enable_bootstrap_user_data || var.user_data_template_path != "") ? base64encode(templatefile(
  ...
  platform = {
    bottlerocket = {
      user_data = var.platform == "bottlerocket" && (var.enable_bootstrap_user_data || var.user_data_template_path != "" || var.bootstrap_extra_args != "") ? base64encode(templatefile(
    ...
    windows = {
      user_data = var.platform == "windows" && var.enable_bootstrap_user_data ? base64encode(templatefile(
...
data "cloudinit_config" "linux_eks_managed_node_group" {
  count = var.platform == "linux" && var.is_eks_managed_node_group && !var.enable_bootstrap_user_data && var.pre_bootstrap_user_data != "" && var.user_data_template_path == "" ? 1 : 0
